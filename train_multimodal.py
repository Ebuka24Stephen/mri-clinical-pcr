"""Train the MRI + clinical (multimodal) pCR model.

Usage::

    python train_multimodal.py --config configs/multimodal.yaml
    python train_multimodal.py --config configs/multimodal.yaml --experiment my_run
"""
...
def main() -> None:
    """Parse arguments, load the config and run the multimodal experiment."""
    parser = argparse.ArgumentParser(description="Train the MRI + clinical pCR model.")
    parser.add_argument(
        "--config",
    ...
    logger.info("Training multimodal model '%s'.", cfg.experiment_name)
